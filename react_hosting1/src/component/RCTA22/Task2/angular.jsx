...
class Angular extends Component {
  ...
  handleOptionChange = (pr) => {
    ...
    this.setState({ course, status }, () => {
      this.callURL("/jobs/angular/1", pr);
    });
  };
  ...
  render() {
    ...
    let resume1 = resume.filter((res) => res.tech === "Angular");
    ...
    const nextPage = () => {
      ...
      const url = `/jobs/angular/${nextPageNum}?${searchString}`;
    ...
    const previousPage = () => {
      ...
      const searchString = queryString.stringify(newQueryParams);
      const url = `/jobs/angular/${previousPageNum}?${searchString}`;
      this.props.history.push(url);
    };
