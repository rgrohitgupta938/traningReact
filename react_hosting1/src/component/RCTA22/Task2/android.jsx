...
class Android extends Component {
  ...
  handleOptionChange = (pr) => {
    ...
    this.setState({ course, status }, () => {
      this.callURL("/jobs/android/1", pr);
    });
  };
  ...
  render() {
    ...
    let resume1 = resume.filter((res) => res.tech === "Android");
    ...
    const nextPage = () => {
      ...
      const url = `/jobs/android/${nextPageNum}?${searchString}`;
    ...
    const previousPage = () => {
      ...
      const searchString = queryString.stringify(newQueryParams);
      const url = `/jobs/android/${previousPageNum}?${searchString}`;
      this.props.history.push(url);
    };
      ...
      <div className="container">
        <div className="row">
          ...
          <div className="col-8">
            ...
            {pageNum > 1 && <button className="btn btn-primary btn-sm m-2" onClick={previousPage}>Previous</button>}
            {pageNum < totalPages && <button className="btn btn-primary btn-sm m-2" onClick={nextPage}>Next</button>}
          </div>
        </div>
      </div>
    );
